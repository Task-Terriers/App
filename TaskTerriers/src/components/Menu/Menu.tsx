...
const Menu: React.FC<MenuComponentProps> = ({
    ...
    if (rightDetail) {
      return (
        <Col mr={rightIconElement ? 12 : 0} justifyCenter>
          <Span labelL numberOfLines={1} colorNeutral40 bold>
            {rightDetail.toString()}
          </Span>
        </Col>
      )
    }
  ...
  const renderRightSide = () => {
    return (
      <Row flexShrink={rightIconElement ? 1 : 0} alignCenter mr8>
        <Col flexShrink={rightDetail?.length > 50 && !rightIconElement ? 0 : 1}>{renderRightDetail()}</Col>
  ...
  const renderButton = () => {
    ...
    if (button?.buttonType == 'universal') {
      ...
      return <WarningButton size="small" icon={button?.icon} text={button?.text} onPress={button?.onPress} warningStyle="fill" />
    }
  }
  ...
  const renderDescription = () => {
    ...
    return (
      <Row>
        <Col>
          <Span bodyM colorNeutral40 bold mt8 numberOfLines={1}>
            {description.toString()}
          </Span>
          {subDescription && (
            <Span bodyM colorNeutral40 bold mt4>
